...
use bdk_wallet::miniscript::miniscript::types;
...
pub type WordCount = bdk_wallet::keys::bip39::WordCount;
...
type Network = bdk_wallet::bitcoin::Network;

#[uniffi::remote(Enum)]
#[uniffi::export(Eq, Hash, Debug)]
#[non_exhaustive]
pub enum Network {
    /// Mainnet Bitcoin.
    Bitcoin,
...
pub type KeychainKind = bdk_wallet::KeychainKind;
...
#[derive(Debug, uniffi::Enum, Clone, PartialEq, Eq, Hash)]
...
impl From<EsploraTxStatus> for TxStatus {
    fn from(status: EsploraTxStatus) -> Self {
        TxStatus {
...
impl From<EsploraTx> for Tx {
    fn from(tx: EsploraTx) -> Self {
        Self {
            ...
            vin: tx.vin.into_iter().map(Vin::from).collect(),
            vout: tx.vout.into_iter().map(Vout::from).collect(),
            size: tx.size as u64,
            weight: tx.weight,
            fee: tx.fee,
