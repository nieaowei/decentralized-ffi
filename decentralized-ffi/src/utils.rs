use crate::bitcoin::{Address, Script, TxIn, TxOut};
...
pub fn new_txout_from_hex(hex: String) -> Option<TxOut> {
    deserialize_hex::<BDKTxOut>(hex.as_str())
        .ok()
        .map(|e| (&e).into())
}
...
#[uniffi::export]
pub fn get_json_info_from_url(
    url: String,
    auth: String,
    params: Vec<String>,
    paths: Vec<String>,
) -> Result<Vec<Option<String>>, UtilsError> {
    let mut url = url;
    ...
    let auths: HashMap<String, String> =
        serde_json::from_str(&auth).map_err(|err| UtilsError::JsonPath {
    ...
    let json = resp
        ...
        .send()
        .map_err(|e| UtilsError::Minreq {
            error_message: e.to_string(),
        })?;
    if json.status_code != 200 {
        return Err(UtilsError::Minreq {
            error_message: json.as_str().unwrap_or_default().to_string(),
        });
    }
    let json = json.as_str().map_err(|e| UtilsError::JsonPath {
        error_message: e.to_string(),
    })?;

    let mut datas = Vec::new();
    for path in paths {
        let path = JsonPath::from_str(path.as_str()).map_err(|e| UtilsError::JsonPath {
            error_message: e.to_string(),
        })?;
        let data =
            path.find(
...
#[uniffi::export]
pub fn extract_contact_from_csv_data(
    csv_data: Vec<u8>,
    network: Network,
) -> Result<Vec<Contact>, UtilsError> {
    let mut csv = csv::Reader::from_reader(csv_data.as_slice());
    let mut contacts = vec![];
    for record in csv.deserialize::<Contact>() {
        match record {
            Ok(contact) => {
                if let Ok(addr) = Address::new(contact.address.clone(), network) {
