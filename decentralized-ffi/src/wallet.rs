...
impl Wallet {
    ...
    pub fn create_single(
        ...
        let create_params = BdkWallet::create_single(descriptor).network(network);
    ...
    pub fn load(
        ...
        let descriptor = descriptor.to_string_with_secret();
        let change_descriptor = change_descriptor.map(|e| e.to_string_with_secret());
        let mut persist_lock = persister.inner.lock().unwrap();
        let deref = persist_lock.deref_mut();
    ...
    pub fn get_txout(&self, op: OutPoint) -> Option<TxOut> {
        self.get_wallet()
            .tx_graph()
            .get_txout(op.into())
    ...
    // pub fn insert_tx(&self, tx: &Transaction) -> ChangeSet<ConfirmationBlockTime> {
    //     self.get_wallet().tx_graph().insert_tx(tx.into())
    //     //             .insert_tx(tx.into())
    // }
