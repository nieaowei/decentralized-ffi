    ...
    for (i, txin) in tx.input().iter().enumerate() {
        if let Some(sign) = txin.witness.first() {
            if let Some(sign_type) = sign.last() {
                if let Ok(t) = TapSighashType::from_consensus_u8(*sign_type) {
                    if t == TapSighashType::SinglePlusAnyoneCanPay {
                        if let Some(txout) = tx.output().get(i) {
                            pair.push(TxInAndTxOut {
                                txin: txin.clone(),
                                txout: txout.clone(),
                            });
                        }
...
pub fn build_rune_snipe_psbt(
    ...
    let snipe_psbt = snipe::SnipeRunePsbtBuilder {
        cardinal_utxos,
        snipe_utxo_pairs: snipe_utxo_pairs
            .into_iter()
            .map(|x| ((&x.txin).into(), (&x.prevout).into(), (&x.txout).into()))
            .collect(),
    ...
    let split_psbt = snipe::SplitRunePsbtBuilder {
        ordi_addr_outpoint_with_amount: (
            ordi_addr.0.clone(),
            bitcoin::OutPoint {
...
pub fn build_inscription_snipe_psbt(
    ...
    let split_psbt = snipe::SplitInscriptionPsbtBuilder {
        ...
        inscription_offsets: snipe_utxo_pairs
            .into_iter()
            .map(|x| x.prevout.value.0)
            .collect(),
...
//        println!(
//            "{:?}",
//            TapSighashType::from_consensus_u8(*back.to_vec().last().unwrap().last().unwrap())
//        );
//    }
//    #[test]
...
//                txin: txin.clone(),
//                prevout: TxOut {
//                    value: outputs[txin.previous_output.vout.clone() as usize]
//                        .value
//                        .clone(),
//                    script_pubkey: outputs[txin.previous_output.vout.clone() as usize]
//                        .script_pubkey
//                        .clone(),
...
//        }
//
//        let recv = bitcoin::Address::from_str(
//            "bc1pfuqd6gadnlycmyas8nc8zgads69uhzhejjvx8epenqa7pcfxqtkqyq6666",
//        )
//        .unwrap()
//        .assume_checked();
//        let ordi = bitcoin::Address::from_str(
//            "bc1pdv25r3a8sq7chv6nxpazcnsvpatwrepk7m0krn07u63c9px9st3sauc8eu",
//        )
//        .unwrap()
//        .assume_checked();
//
...
//            outpoint: OutPoint {
//                txid: Arc::new(
//                    "b5fa21b422ea2bd8ea073cae531d420711fd87c9105fc3a8bb5eae8d46f3794d"
//                        .parse()
//                        .unwrap(),
//                ),
//                vout: 2,
...
//            vec![utxo],
//            snipe_pair,
//            Arc::new(recv.clone().into()),
//            Arc::new(ordi.clone().into()),
...
//            "{}",
//            serialize_hex(
//                psbt.split.extract_tx_unchecked_fee_rate().output()[1]
//                    .script_pubkey
//                    .0
//                    .as_script()
//            )
...
//                        txid: "e689eca28f7be8f813040cee65f49cf5f6ce3b3a0e1d7699bc378d06b7d252f4"
//                            .parse()
//                            .unwrap(),
//                        vout: 0,
//                    },
...
//                "bc1pd5ge4c2e85wad2tyzd4awa6ugkj5g99mg9jq8p0q203rjcnx0r3se32590",
//            )
//            .unwrap()
//            .assume_checked()
//            .script_pubkey(),
//            None,
//            Some(Witness::from(txin.witness.clone())),
//        );
...
//                bitcoin::psbt::Input {
//                    witness_utxo: Some(bitcoin::TxOut {
//                        value: bitcoin::Amount::from_sat(132546),
